...
job_status_t JOB_Process(job_t* JobToProcess){
	switch (JobToProcess->Type) {
		...
		case JOB_TYPE_TRANSMIT:
			if (NODE_TransmitData(JobToProcess->ProcessedNode, JOB_TYPE_TRANSMIT) != NODE_OK) {
				puts("JOB: Error during data sending");
				return JOB_ERROR;
			}
			break;
		case JOB_TYPE_TRANSMIT_START:
			if (NODE_StartTransmitData(JobToProcess->ProcessedNode, JOB_TYPE_TRANSMIT_START) != NODE_OK) {
				puts("JOB: Error during data sending");
				return JOB_ERROR;
			}
			break;
		case JOB_TYPE_RETRANSMIT:
			if (NODE_TransmitData(JobToProcess->ProcessedNode, JOB_TYPE_RETRANSMIT) != NODE_OK) {
				puts("JOB: Error during data sending");
				return JOB_ERROR;
			}
			break;
		case JOB_TYPE_RETRANSMIT_START:
			if (NODE_StartTransmitData(JobToProcess->ProcessedNode, JOB_TYPE_RETRANSMIT_START) != NODE_OK) {
				puts("JOB: Error during data sending");
				return JOB_ERROR;
			}
			break;
